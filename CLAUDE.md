...
| Verification | 629 tests, 441 static plus 188 live. Blocks 1 to 5 reviewed 2026-09-20, ten defects found and fixed, each with a regression test. Second review 2026-09-21 found eight more, none fixed yet, five tracked as it.fails in tests/policy/live-review2-regressions.test.ts, see section 6. Live layer runs as anon and authenticated against Postgres and against PostgREST with a real anon key. Grants verified against the engine, not the SQL text. CI runs the static layer only, the live layer skips without a stack |
...
| Open defect | Second review 2026-09-21, eight findings unfixed. publish_claim re-claims parked jobs and freezes the first revision, publish_resolve has no stage fence, app_lock_identity raises on an empty key set, an alert event with zero deliveries is never reconciled, reconcile_alert_event has a read then write race, publish_commit copies captured_geo into display_geo verbatim, generate-tokens.ts carries a dead replace. Awaiting CKC go on fixes |
...
| display_geo derivation. publish_commit sets display_geo to captured_geo verbatim, so the private value is the public value. MVP section 12.1 says raw captured coordinates are never public by default, no doc says how display_geo derives. Options, null until a rule exists, airport centroid from airport_icao, or rounded to 2 decimals. Recommendation, null for Phase S | CKC | Phase 0 release, invariant 6 |
